...
class CreateNewUser implements CreatesNewUsers
{
    ...
    public function create(array $input)
    {
        ...
        // Create crypto account with default balances
        $cryptoaccnt = CryptoAccount::create(array_merge(
            ['user_id' => $user->id],
            CryptoAccount::getDefaultBalances()
        ));
        $request->session()->forget('ref_by');
        try {
            Mail::to($user->email)->send(new WelcomeEmail($user));
        } catch (Throwable $e) {
            Log::error('Welcome email failed to send during registration', [
                'user_id' => $user->id,
                'email' => $user->email,
                'exception' => $e,
            ]);
