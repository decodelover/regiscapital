...

{
    ...
    {
        $methodname = Wdmethod::findCuratedDepositMethod($id);
        if (!$methodname) {
    ...
    public function newdeposit(Request $request)
    {
        ...
        $methodname = $this->resolveDepositMethod($request);

        if (empty($methodname)) {
            return redirect()->back()->with('message', 'Invalid deposit method. Allowed methods: Bank Transfer, Bitcoin, Ethereum, or USDT.');
        }

        if (!is_numeric($request->amount) || (float) $request->amount <= 0) {
            return redirect()->back()->with('message', 'Please enter a valid deposit amount.');
        }
        ...
        $request->session()->put('payment_method_id', $methodname->id);
    ...
    {
        $methodId = $request->session()->get('payment_method_id');
        $methodname = Wdmethod::findCuratedDepositMethod($methodId ?: $request->session()->get('payment_mode'));

        if (!$methodname) {
            return redirect()->route('deposits')->with('message', 'Please select a valid deposit method.');
        }
    ...
    public function savestripepayment(Request $request)
    {
        ...
        $request->session()->forget('payment_method_id');
    ...
    public function savedeposit(Request $request)
    {
        ...
        $isBlocked = in_array($userStatus, ['blocked', 'suspended', 'disabled'], true);

        if ($isBlocked) {
            return redirect()->back()
                ->with("message", "Sorry, your account is restricted. Contact support on $settings->contact_email for more details.");
        }

        $this->validate($request, [
            'amount' => 'required|numeric|min:1',
            'paymethd_method_id' => 'nullable|numeric',
            'paymethd_method' => 'nullable|string',
            'proof' => 'required|file|mimes:jpg,jpeg,png,pdf|max:5120',
        ]);

        $paymentMethod = Wdmethod::findCuratedDepositMethod(
            $request->input('paymethd_method_id', $request->input('paymethd_method'))
        );

        if (empty($paymentMethod)) {
            return redirect()->back()->with('message', 'Selected deposit method is not available.');
        }
...
//Save deposit
        $dp = new Deposit();
        $dp->amount = $request['amount'];
        $dp->payment_mode = $paymentMethod->name;
        $dp->status = 'Pending';
        $dp->proof = $path;
        ...
        NotificationHelper::create(
            $user,
            'Your deposit of ' . $settings->currency . $request['amount'] . ' via ' . $paymentMethod->name . ' has been received and is pending approval.',
            'Deposit Submitted',
            'info',
            'upload',
            route('deposits')
    ...
    /**
     * Resolve a valid curated deposit method from the incoming request.
     */
    private function resolveDepositMethod(Request $request): ?Wdmethod
    {
        $methodId = $request->input('payment_method_id');
        $methodName = $request->input('payment_method');

        return Wdmethod::findCuratedDepositMethod($methodId ?: $methodName);
    }
