...
class ViewsController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $monthly_deposits = DB::table('deposits')
            ->where('user', $user->id)
            ->where('status', 'Processed')
            ->whereBetween('created_at', [$startOfMonth, $endOfMonth])
            ->sum('amount');
        $monthly_expenses = DB::table('withdrawals')
            ->where('user', $user->id)
            ->where('status', 'Processed')
            ->whereBetween('created_at', [$startOfMonth, $endOfMonth])
            ->sum('amount');
        ...
        // Get KYC status
        $kycStatus = $user->account_verify;
        $normalizedKycStatus = strtolower((string) $kycStatus);
        $kycStatusIcon = match($normalizedKycStatus) {
            'verified' => 'check-circle',
            'under review' => 'clock',
            'rejected' => 'x-circle',
            default => 'alert-circle'
        };
        $kycStatusColor = match($normalizedKycStatus) {
            'verified' => 'emerald',
            'under review' => 'amber',
            'rejected' => 'rose',
            default => 'gray'
...
public function internationaltransfer()
{
    ...
    {
        $paymethod = Wdmethod::curatedDepositMethods();
        $requiredDepositMethods = Wdmethod::curatedDepositKindsStatus();
        ...
        return view('user.deposits')
            ->with(array(
                ...
                'dmethods' => $paymethod,
                'requiredDepositMethods' => $requiredDepositMethods,
