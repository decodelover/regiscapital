...
class VirtualCardController extends Controller
{
    ...
    public function approveCard(Request $request, $id)
    {
        ...
        if ($card->status !== 'pending') {
            return redirect()->back()->with('error', 'Only pending card requests can be approved.');
        }
        ...
        DB::transaction(function () use ($card, $user) {
            ...
            $card->card_holder_name = $card->card_holder_name ?: trim($user->name . ' ' . ($user->lastname ?? ''));
            ...
            $card->card_pan = $cardDetails['card_pan'];
            $card->card_token = $cardDetails['card_token'];
            $card->monthly_limit = $card->monthly_limit ?: ((float) $card->daily_limit * 30);
            $card->approval_date = now();
            $card->status = 'active';
            $card->save();
        });
    ...
    public function rejectCard(Request $request, $id)
    {
        ...
        if ($card->status !== 'pending') {
            return redirect()->back()->with('error', 'Only pending card requests can be rejected.');
        }
        ...
        DB::transaction(function () use ($card, $user, $request) {
            // Update card status and review note
            $card->status = 'rejected';
            $card->rejection_reason = $request->rejection_reason;
            $card->save();

            // Refund issuance fee if it was charged at application time.
            $feeTransaction = CardTransaction::where('card_id', $card->id)
            ...
            if ($feeTransaction) {
                $refundReferenceSuffix = (string) $feeTransaction->transaction_reference;
                $alreadyRefunded = CardTransaction::where('card_id', $card->id)
                    ->where('transaction_type', 'refund')
                    ->where('description', 'like', '%' . $refundReferenceSuffix . '%')
                    ->exists();

                if (!$alreadyRefunded) {
                    $refundAmount = abs((float) $feeTransaction->amount);
                    $user->account_bal = (float) $user->account_bal + $refundAmount;
                    $user->save();
                    ...
                    CardTransaction::create([
                        ...
                        'transaction_reference' => 'RFD' . strtoupper(Str::random(8)),
                        'merchant_name' => config('app.name'),
                        'status' => 'completed',
                        'description' => 'Issuance fee refund for rejected application (' . $refundReferenceSuffix . ')',
        ...
        $reasonText = $card->rejection_reason ? (' Reason: ' . $card->rejection_reason) : '';

        // Create notification
        NotificationHelper::create(
            $user,
            'Your ' . ucfirst($card->card_level) . ' ' . ucfirst(str_replace('_', ' ', $card->card_type)) . ' card request was declined after review.' . $reasonText,
    ...
    public function blockCard($id)
    {
        ...
        if (!in_array($card->status, ['active', 'inactive'], true)) {
            return redirect()->back()->with('error', 'Only active or inactive cards can be blocked.');
        }
    ...
    public function unblockCard($id)
    {
        ...
        if ($card->status !== 'blocked') {
            return redirect()->back()->with('error', 'Only blocked cards can be unblocked.');
        }
    ...
    public function topupCard(Request $request, $id)
    {
        ...
        if (!in_array($card->status, ['active', 'inactive'], true)) {
            return redirect()->back()->with('error', 'Only active or inactive cards can be topped up.');
        }
    ...
    public function deductCard(Request $request, $id)
    {
        ...
        if (!in_array($card->status, ['active', 'inactive'], true)) {
            return redirect()->back()->with('error', 'Only active or inactive cards can be debited.');
        }
    ...
    private function generateCardDetails($cardType)
    {
        ...
        if (strpos(strtolower($cardType), 'visa') !== false) {
            ...
            $cardNumber = $bin . $this->generateRandomDigits(10);
        } elseif (strpos(strtolower($cardType), 'american_express') !== false || strpos(strtolower($cardType), 'amex') !== false) {
    ...
    public function updateSettings(Request $request)
    {
        ...
        $cardSettings = CardSettings::firstOrCreate(['id' => 1]);
