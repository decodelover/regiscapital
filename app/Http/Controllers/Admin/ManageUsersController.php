    ...
    public function markplanas($status, $id)
    {
        ...
        if ($plan && $plan->user) {
            ...
            if ($user) {
                ...
                // Send loan status email to user
                try {
                    $latestPlan = User_plans::find($plan->id);
                    if ($latestPlan) {
                        Mail::to($user->email)->send(new LoanStatus($user, $latestPlan, $status, $title));
                    }
                } catch (\Exception $e) {
                    Log::warning('Loan status email failed to send', [
                        'user_id' => $user->id,
                        'plan_id' => $plan->id,
                        'status' => $status,
                        'error' => $e->getMessage(),
